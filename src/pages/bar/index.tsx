...
import { BAR_ADDRESS, ChainId, SUSHI, Token, ZERO } from '@sushiswap/core-sdk'
import Button from 'app/components/Button'
import Container from 'app/components/Container'
import Dots from 'app/components/Dots'
import Input from 'app/components/Input'
import { XOPR } from 'app/config/tokens'
...
export default function Stake() {
  ...
  const OPR = new Token(
    ChainId.FANTOM,
    '0x16dbd24713c1e6209142bcfeed8c170d83f84924',
    18,
    'OPR',
    'fantoms.art Token'
  )
  const sushiBalance = useTokenBalance(account ?? undefined, OPR)//SUSHI[ChainId.ETHEREUM])
  const xOprBalance = useTokenBalance(account ?? undefined, XOPR)
  const xOprAddress = {
    250: '0x9e7f8d1e52630d46e8749c8f81e91d4e934f3589'
  }

  const { enter, leave, xOPRForOPR } = useSushiBar()
  ...
  const balance = activeTab === 0 ? sushiBalance : xOprBalance

  const formattedBalance = balance?.toSignificant(4)

  const parsedAmount = usingBalance ? balance : tryParseAmount(input, balance?.currency)

  const [approvalState, approve] = useApproveCallback(parsedAmount, xOprAddress[ChainId.FANTOM])
  ...
  const xOpr = useTokens({
    ...
    variables: { where: { id: XOPR.address.toLowerCase() } },
  })?.[0]

  const bar = useBar()

  //const [xOprPrice] = [xOpr?.derivedETH * ethPrice, xOpr?.derivedETH * ethPrice * bar?.totalSupply]


  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake OPR</title>
        <meta
          key="description"
          name="description"
          content="Stake OPR in return for xOPR, an interest bearing and fungible ERC20 token designed to share revenue generated by all Fantoms On Opera products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://staking.fantoms.art/" />
        <meta key="twitter:title" name="twitter:title" content="STAKE OPR" />
        <meta
          key="twitter:description"
          name="twitter:description"
          content="Stake OPR in return for xOPR, an interest bearing and fungible ERC20 token designed to share revenue generated by all Fantoms On Opera products."
        />
        <meta key="twitter:image" name="twitter:image" content="https://cyber.fantoms.art/Opr.png" />
        <meta key="og:title" property="og:title" content="STAKE OPR" />
        <meta key="og:url" property="og:url" content="https://staking.fantoms.art/stake" />
        <meta key="og:image" property="og:image" content="https://cyber.fantoms.art/Opr.png" />
        <meta
          key="og:description"
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking OPR for xOPR`)}
              </div>
            ...
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`The treasury buys back OPR and sends it to the xOPR Lab. When your OPR is staked into the Lab, you receive
                                xOPR in return for voting rights and a fully composable token that can interact with other protocols.
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake OPR`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xOPR gradually equals more OPR`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}OPR`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <img
                      ...
                      src="https://staking.fantoms.art/xopr.png"
                      alt="xOPR symbol"
                      width="64"
                      height="64"
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xOprBalance ? xOprBalance.toSignificant(4) : '~'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xOPR</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <img
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="https://cyber.fantoms.art/Opr.png"
                      alt="OPR symbol"
                      width="64"
                      height="64"
                    />
                    <div className="flex flex-col justify-center">
                      ...
                      <p className="text-sm md:text-base tex t-primary">OPR</p>
                    </div>
                  </div>
                </div>
